...
HCOSE_SIGN0 _COSE_Sign0_Init_From_Object(cn_cbor *cbor, COSE_Sign0Message *pIn, CBOR_CONTEXT_COMMA cose_errback *perr)
{
    ...
    if (pobj == NULL) {
        pobj = (COSE_Sign0Message *)COSE_CALLOC(1, sizeof(COSE_Sign0Message), context);
    }
...
errorReturn:
    ...
    if (!CreateSign0AAD(pSigner, &pbToSign, &cbToSign, "Signature1", perr)) {
        goto errorReturn;
    }
...
bool _COSE_Signer0_validate(COSE_Sign0Message *pSign, const cn_cbor *pKey, cose_errback *perr)
{
    ...
    if (!CreateSign0AAD(pSign, &pbToSign, &cbToSign, "Signature1", perr)) {
        goto errorReturn;
    }
...
#ifdef USE_ECDSA_SHA_256
        case COSE_Algorithm_ECDSA_SHA_256:
            if (!ECDSA_Verify(&pSign->m_message, INDEX_SIGNATURE + 1, pKey, 256, pbToSign, cbToSign, perr)) {
                goto errorReturn;
            }
            break;
#endif
...
#ifdef USE_ECDSA_SHA_384
        case COSE_Algorithm_ECDSA_SHA_384:
            if (!ECDSA_Verify(&pSign->m_message, INDEX_SIGNATURE + 1, pKey, 384, pbToSign, cbToSign, perr)) {
                goto errorReturn;
            }
            break;
#endif
...
#ifdef USE_ECDSA_SHA_512
        case COSE_Algorithm_ECDSA_SHA_512:
            if (!ECDSA_Verify(&pSign->m_message, INDEX_SIGNATURE + 1, pKey, 512, pbToSign, cbToSign, perr)) {
                goto errorReturn;
            }
            break;
#endif
