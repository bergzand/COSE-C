...
HCOSE_SIGN _COSE_Sign_Init_From_Object(cn_cbor *cbor, COSE_SignMessage *pIn, CBOR_CONTEXT_COMMA cose_errback *perr)
{
    ...
    if (pobj == NULL) {
        pobj = (COSE_SignMessage *)COSE_CALLOC(1, sizeof(COSE_SignMessage), context);
    }
    CHECK_CONDITION(pobj != NULL, COSE_ERR_OUT_OF_MEMORY);
...
HCOSE_SIGNER COSE_Sign_add_signer(HCOSE_SIGN hSign, const cn_cbor *pkey, int algId, cose_errback *perr)
{
    ...
    if (!COSE_Signer_map_put_int(hSigner, COSE_Header_Algorithm, cbor2, COSE_PROTECT_ONLY, perr)) {
        goto errorReturn;
    }
    cbor2 = NULL;
    ...
    if (cbor != NULL) {
        ...
        CHECK_CONDITION_CBOR(cbor2 != NULL, cbor_error);
        if (!COSE_Signer_map_put_int(hSigner, COSE_Header_KID, cbor2, COSE_UNPROTECT_ONLY, perr)) {
            goto errorReturn;
        }
        cbor2 = NULL;
    }
