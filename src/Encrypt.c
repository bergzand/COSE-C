...
HCOSE_ENVELOPED _COSE_Enveloped_Init_From_Object(cn_cbor *cbor, COSE_Enveloped *pIn, CBOR_CONTEXT_COMMA cose_errback *perr)
{
    ...
    if (pobj == NULL) {
        pobj = (COSE_Enveloped *)COSE_CALLOC(1, sizeof(COSE_Enveloped), context);
    }
    if (pobj == NULL) {
...
errorReturn:
    ...
    if (pbKeyIn != NULL) {
        ...
        if (pRecip != NULL) {
            ...
            for (pRecipX = pcose->m_recipientFirst; pRecipX != NULL; pRecipX = pRecipX->m_recipientNext) {
                if (pRecipX == pRecip) {
                    if (!_COSE_Recipient_decrypt(pRecipX, pRecip, alg, cbitKey, pbKeyNew, perr)) {
                        goto errorReturn;
                    }
                    break;
                }
            ...
            for (pRecip = pcose->m_recipientFirst; pRecip != NULL; pRecip = pRecip->m_recipientNext) {
                if (_COSE_Recipient_decrypt(pRecip, NULL, alg, cbitKey, pbKeyNew, perr)) {
                    break;
                }
            }
    ...
    if (!_COSE_Encrypt_Build_AAD(&pcose->m_message, &pbAuthData, &cbAuthData, szContext, perr)) {
        goto errorReturn;
    }
...
#ifdef USE_AES_CCM_16_64_128
        case COSE_Algorithm_AES_CCM_16_64_128:
            if (!AES_CCM_Decrypt(pcose, 64, 16, pbKey, cbitKey / 8, cn->v.bytes, cn->length, pbAuthData, cbAuthData, perr)) {
                goto error;
            }
            break;
#endif
...
#ifdef USE_AES_CCM_16_128_128
        case COSE_Algorithm_AES_CCM_16_128_128:
            if (!AES_CCM_Decrypt(pcose, 128, 16, pbKey, cbitKey / 8, cn->v.bytes, cn->length, pbAuthData, cbAuthData, perr)) {
                goto error;
            }
            break;
#endif
...
#ifdef USE_AES_CCM_64_64_128
        case COSE_Algorithm_AES_CCM_64_64_128:
            if (!AES_CCM_Decrypt(pcose, 64, 64, pbKey, cbitKey / 8, cn->v.bytes, cn->length, pbAuthData, cbAuthData, perr)) {
                goto error;
            }
            break;
#endif
...
#ifdef USE_AES_CCM_64_128_128
        case COSE_Algorithm_AES_CCM_64_128_128:
            if (!AES_CCM_Decrypt(pcose, 128, 64, pbKey, cbitKey / 8, cn->v.bytes, cn->length, pbAuthData, cbAuthData, perr)) {
                goto error;
            }
            break;
#endif
...
#ifdef USE_AES_GCM_128
        case COSE_Algorithm_AES_GCM_128:
            if (!AES_GCM_Decrypt(pcose, pbKey, cbitKey / 8, cn->v.bytes, cn->length, pbAuthData, cbAuthData, perr)) {
                goto error;
            }
            break;
#endif
...
#ifdef USE_AES_CCM_16_64_128
        case COSE_Algorithm_AES_CCM_16_64_128:
            if (!AES_CCM_Encrypt(pcose, 64, 16, pbKey, cbKey, pbAuthData, cbAuthData, perr)) {
                goto errorReturn;
            }
            break;
#endif
...
#ifdef USE_AES_CCM_16_128_128
        case COSE_Algorithm_AES_CCM_16_128_128:
            if (!AES_CCM_Encrypt(pcose, 128, 16, pbKey, cbKey, pbAuthData, cbAuthData, perr)) {
                goto errorReturn;
            }
            break;
#endif
...
#ifdef USE_AES_CCM_64_64_128
        case COSE_Algorithm_AES_CCM_64_64_128:
            if (!AES_CCM_Encrypt(pcose, 64, 64, pbKey, cbKey, pbAuthData, cbAuthData, perr)) {
                goto errorReturn;
            }
            break;
#endif
...
#ifdef USE_AES_CCM_64_128_128
        case COSE_Algorithm_AES_CCM_64_128_128:
            if (!AES_CCM_Encrypt(pcose, 128, 64, pbKey, cbKey, pbAuthData, cbAuthData, perr)) {
                goto errorReturn;
            }
            break;
#endif
...
#ifdef USE_AES_GCM_128
        case COSE_Algorithm_AES_GCM_128:
            if (!AES_GCM_Encrypt(pcose, pbKey, cbKey, pbAuthData, cbAuthData, perr)) {
                goto errorReturn;
            }
            break;
#endif
